...
class LlamaInstructSplitter(BaseSplitter):
	...
	Parameters
	----------
	use_german_prompts : `bool`, optional, defaults to `False`
		if `True`, German versions of the prompts and tools will be used,
		otherwise English versions

	n_shots : `int`, optional, defaults to `0`
		number of examples for a few-shot setup in the main prompt

	suggest_candidates : `bool`, optional, defaults to `True`
		whether to add a pipeline node to use a summary of German linking element tendencies
		to generate a list of preliminary first constituent candidates based on that

	 retrieve_paradigm : `bool`, optional, defaults to `False`
	 	whether to add a pipeline node to suggest preliminary candidates and
		call a tool to lookup those in Wiktionary to retrieve their paradigms;
		the paradigm infos are then matched against the summary of German linking element tendencies
		to base the final first constituent candidate generation based on them;
		requires `suggest_candidates=True`

	max_generations : `int`, optional, defaults to `3`
		maximum number of generations in case generation fails (e.g. due to invalid JSON)
		or impossible analysis is generated; the first generation is included into `max_generations`

	log_messages : `bool`, optional, defaults to `True`
		if `True`, all generations are stored and available in the `messages_log` attribute

	save_graph : `bool`, optional, defaults to `False`
		whether to save the workflow graph (generated by LangGraph); if `True`, binary representation
		of the graph in PNG will be stored in the `plot_buffer` attribute
